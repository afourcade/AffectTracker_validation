...
Python code (in the structure of a python package) is stored in `./code/affecttracker_validation/`
...
*Initialize a new R-project in the project root of `affecttracker_validation` with `RStudio`.
...
Both files will be read out by the script in `./code/affecttracker_validation/configs.py`.
...
```python
from affecttracker_validation.configs import config, paths
