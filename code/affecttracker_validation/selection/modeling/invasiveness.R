...
# read data
data_sr <- read.csv(paste(data_path, "cri_sr_clean.csv", sep = ""))
...
Results_path_invq <- "E:/AffectiveVR/Phase_1/affectivevr/assessment_results/invasiveness/questionnaire/"

# plotting per rating method
p_invq <- plot_rm_q(df=invq, x=rating_method, y=response, fill=rating_method, xlab="Rating Method", ylab="Invasiveness", breaks_y=seq(0,6,1), limits_y = c(0,6), savepath=file.path(Results_path_invq, "invasiveness_rating_method.png"))
p_invq
...
test_site.anova <- anova_test(invq, dv = response, wid = sj_id, between = test_site, effect.size = "pes")
get_anova_table(test_site.anova)

# Plotting per test site
plot_rm_q(df=invq, x=test_site, y=response, fill=test_site, xlab="Test_site", ylab="Invasiveness", breaks_y=seq(0,6,1), savepath=file.path(Results_path_invq, "invasiveness_testing_site.png"))

# Tost approach
# Extract data for each test site
test_site1 <- (invq %>% filter(test_site == test_list[1]))$response
test_site2 <- (invq %>% filter(test_site == test_list[2]))$response
...
# save dataframe in csv file
write.csv(data.frame(test_site1_test_site2_test$TOST), file = file.path(Results_path_invq, "TOST_test_site.csv"))

#save results in txt file
sink(file = file.path(Results_path_invq, "results_compare_test_site.txt"))
