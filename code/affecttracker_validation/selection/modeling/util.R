...
# plot_rm_q ------------------------------------------------------------------
plot_rm_q <- function(df, x, y, fill, xlab, ylab, breaks_y, limits_y, savepath) {
  ...
  # - limits_y: Numeric vector specifying the limits on the y-axis
  ...
  p <- ggplot(df, aes(x = {{x}}, y = {{y}}, fill = {{fill}})) +
    ...
    # Add jittered points
    geom_point(aes(color = as.numeric(as.character(sj_idx))), position = position_jitter(width = .1, height = 0), size = 1.5) +
    # Add a boxplot
    geom_boxplot(width = .1, alpha = 0.5) +
    # Set the y-axis labels and breaks
    scale_y_continuous(name = ylab, breaks = breaks_y, limits=limits_y) +
    # Set the x-axis label
    #xlab(xlab) +
    xlab('') +
    # Set the color scale legend
    scale_color_continuous(name = "SJ index") +
