...
resultpath = phase_path + 'MBB2024_poster/'
# resultpath = (
#     "/Users/Lucy/Documents/Berlin/FU/MCNB/Praktikum/MPI_MBE/AVR/results/"
# )  # path where results should be saved
phases = ["Phase_1"]  # phases for which the radar chart should be plotted
# "phase2"
questionnaires = [
    "emo_rep",
    "invasiveness",
...
def plot_radar(data: pd.DataFrame, questionnaires: list, rating_methods: list) -> plt.figure():
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            ...
            title=go.layout.Title(text="Mean scores across participants", font=dict(size=20)),
            polar=dict(
                radialaxis=dict(
                    visible=True,
                    range=[0, 100],
            ...
            paper_bgcolor='rgba(0,0,0,0)',  # set background color to fit other plots on the poster
...
if __name__ == "__main__":
    for phase in phases:
        ...
        # reorder questionnaires and data for the plot
        questionnaires = ["sus_score", "presence_score", "invasiveness", "satisfaction", "emo_rep"]
        questionnaire_labels = [
            "System Usability Scale",
            "Presence",
            "Invasiveness",
        ...
        normalized_data = normalized_data.set_index('questionnaire').loc[questionnaires].reset_index()
        radar_chart = plot_radar(normalized_data, questionnaires, rating_methods)

        resultpath_phase_datavisualization = resultpath
        ...
        radar_chart.write_image(resultpath_phase_datavisualization + "radar_chart.svg", scale = 4)
